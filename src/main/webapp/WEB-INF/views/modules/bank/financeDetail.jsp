...
<div class="control-group">
    <h4><c:if test="${finances.type eq '1'}">存款明细</c:if>
        <c:if test="${finances.type eq '2'}">取款明细</c:if></h4>
</div>

<form:form id="searchForm" modelAttribute="finance" action="${ctx}/bank/finance/detail" method="post" class="breadcrumb form-search">
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
    <thead><tr><th><c:if test="${finances.type eq '1'}">存款</c:if>
        <c:if test="${finances.type eq '2'}">取款</c:if>流水号</th><th><c:if test="${finances.type eq '1'}">存款</c:if>
        <c:if test="${finances.type eq '2'}">取款</c:if>资金</th><th><c:if test="${finances.type eq '1'}">存款</c:if>
        <c:if test="${finances.type eq '2'}">取款</c:if>时间</th></tr></thead>
    <tbody>
