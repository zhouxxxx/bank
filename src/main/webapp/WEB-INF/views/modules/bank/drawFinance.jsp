            ...
            if (type == '2') {
                $("#inputForm").validate({
                    rules: {
                        money: {
                            required: true,
                            remote: ctx + "/bank/finance/checkMoney?userId="+$("#userId").val()
                            }
                        },
                        messages: {
                            money: {remote: "取款金额大于余额"}
                        },
...
<h1 style="margin-left: 250px"><c:if test="${finance.type eq '1'}">存款</c:if>
    <c:if test="${finance.type eq '2'}">取款</c:if></h1>
<form:form id="inputForm" modelAttribute="financeDetail" action="${ctx}/bank/finance/save" method="post" class="form-horizontal">
    ...
    <div class="control-group">
        <label class="control-label"><c:if test="${finance.type eq '1'}">存款金额:</c:if>
            <c:if test="${finance.type eq '2'}">取款金额:</c:if></label>
        <div class="controls">
            <input name="money" id="money"  htmlEscape="false" maxlength="50" class="required"/>
            <span class="help-inline"><font color="red">账户余额:${finance.money}</font> </span>
        </div>
    </div>
    <div class="form-actions">
        <input id="btnSubmit" class="btn btn-primary" type="submit" value="<c:if test="${finance.type eq '1'}">存 款</c:if>
<c:if test="${finance.type eq '2'}">取 款</c:if>"/>&nbsp;
